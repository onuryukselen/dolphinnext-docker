...
```
docker run --privileged -m 10G -p 8080:80 -v /path/to/mount:/export -ti ummsbiocore/dolphinnext-docker /bin/bash
```
or with R markdown support;
```
docker run --privileged -m 10G -p 8080:80 -v /path/to/mount:/export -ti ummsbiocore/dolphinnext-studio /bin/bash
```
